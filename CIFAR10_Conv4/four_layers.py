...
class CIFAR10Network:
  ...
  def build(self, X, y, dropout=1, batch_size=32):
    ...
    # print 'CONV3 reshaped', fc1.get_shape()
...
if __name__ == '__main__':
  ...
  y_ = tf.placeholder(tf.int32, [batch_size, num_classes])
  ...
  with tf.Session() as sess:
    ...
    try:
      saver.restore(sess, 'cifar_network.ckpt')
    ...
    for epoch in xrange(num_epochs): # each CIFAR10 batch run num_epoch times
      ...
      for cifar_batch_no in range(1,6): # one single CIFAR10 batch
        ...
        f = os.path.join('cifar-10-batches-py', 'data_batch_%d' % (cifar_batch_no, ))
        X, y = load_CIFAR_batch(f)
  ...
  with tf.Session() as sess:
    ...
    for step in xrange(num_steps):
      ...
      predictions = cifar_network.build(X_batch, y_batch, keep_prob)
      loss = cifar_network.get_loss(predictions, y_batch)
      accuracy = tf.reduce_mean(tf.cast(predictions == y_test_batch, tf.float32))
      l, acc, _ = sess.run([loss, accuracy, optimizer], feed_dict={X_:X_batch, y_:y_batch, keep_prob:1})
      loss_history.append(l)
      acc_history.append(acc)
